...
- **Gegengeprüft am 18.09.2026, es baut.** NestJS 12 übersetzt unter TypeScript 7.0.2 mit `emitDecoratorMetadata`, die Metadaten stehen im Ergebnis, und der Nest-Container löst eine Konstruktorabhängigkeit damit auf. Die vorsorglich angekündigte Ausnahme für `server` entfällt, alle Pakete liegen auf derselben Reihe. Der Test bleibt als `packages/server/src/decorator-metadata.test.ts` stehen, damit ein späterer Versionssprung das nicht unbemerkt kaputt macht.
- **TypeScript 7.0 bringt keine Programmierschnittstelle mit.** Das war die Stelle, an der es tatsächlich geklemmt hat, und sie betrifft jedes Werkzeug, das den Compiler ansteuert statt ihn aufzurufen. typescript-eslint bricht mit einer eindeutigen Meldung ab, statt still weiterzuprüfen. Der Übergang läuft über die Kompatibilitätsfassung, die das TypeScript-Team dafür veröffentlicht hat, eingehängt über zwei Aliase im Wurzelpaket: `tsc` ist TypeScript 7, und unter dem Modulnamen `typescript` liegt die 6.0-Schnittstelle, die der Linter liest. Geprüft wird mit 7, geparst mit 6. Solange nur Regeln ohne Typinformation laufen, und das ist heute der Fall, sehen beide dasselbe. Mit TypeScript 7.1 soll eine neue Schnittstelle kommen; dann fallen die Aliase weg. Wer typinformierte Regeln einschalten will, klärt vorher diesen Punkt, sonst prüft der Linter gegen einen anderen Typchecker als die CI.
...
- **Nachtrag vom 24.09.2026, Releases.** Seit #155 erscheint eine Fassung mit einem Tag auf `main`, und der Workflow "Release" baut daraus die beiden Abbilder, die eine Installation betreibt. Drei Werkzeuge kommen dazu, keines mit einem eigenen Konto oder einem Schlüssel, den jemand aufbewahren müsste. Die Abbilder liegen in `ghcr.io/opengewerk`, weil diese Registry zur Organisation gehört und der Workflow mit seinem eigenen Token hineinschreibt; Docker Hub schied aus, weil der Namensraum `opengewerk` dort niemandem im Projekt gehört (am 24.09.2026 nicht vergeben), und ein Compose, das ein Abbild nicht findet, fragt vor dem Bauen die Registry. Bis #155 hießen die Abbilder `opengewerk/opengewerk` und `opengewerk/backup`, also genau dort. Signiert wird mit cosign über Sigstore, ohne Schlüssel, mit der Identität des Workflows aus dem OIDC-Token von GitHub Actions; `cosign verify` prüft damit, dass ein Abbild aus diesem Workflow dieses Repositorys stammt. Die Aktionen von außerhalb GitHubs stehen in diesem Workflow auf einem Commit statt auf einem Tag, weil der Lauf Abbilder schreiben darf und die Identität hält, mit der sie signiert werden; ein Tag zeigt dahin, wohin sein Besitzer ihn als Nächstes setzt. Dependabot zieht die Commits nach. ARM64 entsteht neben x86_64 über QEMU auf demselben Runner: gemessen am 24.09.2026 baut die Anwendung für arm64 unter Emulation in anderthalb Minuten, eigene ARM-Runner und das Zusammenführen zweier Läufe lohnen sich dafür nicht. Die Fassung steht im Paket in `docker/compose.yaml` an jedem Abbild und nicht in einer Datei daneben, denn nur so nehmen auch die Befehle für Sicherung und Rückspielen, die jemand von Hand eingibt, dieselbe Fassung wie `start.sh`.
