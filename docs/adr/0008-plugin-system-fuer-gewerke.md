...
- **Nachtrag vom 24.09.2026, das erste Paket.** Mit #78 und #79 steht `packages/gewerke/elektro/` als `@opengewerk/gewerk-elektro`: `manifest.json` mit Name, Titel, Fassung, der ältesten Kernfassung und den Dateien unter `formulare/` und `regeln/`, dazu `src/index.ts`, das sie als Werte ausgibt (`elektroForms`, `elektroRules`, `elektroRegistry`). Die JSON-Dateien importiert der Code mit `with { type: 'json' }`, der Compiler legt sie neben das gebaute JavaScript, und Server und Oberfläche bündeln sie so, wie der vorige Nachtrag es verlangt. Die Tests des Pakets prüfen, was ein Beitrag ohne Code falsch machen kann: eine Definition, die nicht zusammenpasst, einen Grenzwert, der eine Regel nennt, die das Paket nicht hat oder in einer Einheit, die nicht zum Messwert passt, und ein Manifest, das Dateien nennt, die es nicht gibt. Die Grenzwerte tragen jeder eine Fundstelle und stehen wie die Regelpakete des Rechts unter Vorbehalt der fachkundigen Abnahme (#31). Eine Formulardefinition wird nicht geändert, sondern bekommt eine neue Datei mit der nächsten Fassung; die alte bleibt im Paket, weil ein Protokoll mit der Fassung gelesen wird, in der es ausgefüllt wurde.
