...
- **Nachtrag vom 18.09.2026, Stammdaten anlegen braucht trotzdem das Recht.** Punkt 4 stellt sich vor, dass ein Monteur vor Ort einen Kunden anlegt. Die Abgleichregel erlaubt das auch, aber das Rechtemodell aus ADR 0006 gibt dem Monteur nur `customer.read`, und das gilt: eine Warteschlange ist ein anderer Weg hinein, keine andere Sache. Ob der Monteur ein eigenes Recht zum Anlegen bekommen soll, ist eine Frage an das Rechtemodell und nicht an den Abgleich; sie gehört zu dem Issue, das die Rollen erweitert, und wird dort entschieden.
